    ...
    <div className="min-h-screen bg-[#f4f6f3]">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-black/10 bg-neutral-950 p-5 text-white lg:block">
        ...
        <div className="absolute bottom-5 left-5 right-5 rounded-lg bg-white/10 p-4 text-sm text-white/72">
          ...
          Configure Supabase env vars to enforce real admin login in production.
        </div>
      </aside>
      <div className="lg:pl-72">
        <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between border-b border-black/10 bg-white/90 px-5 backdrop-blur">
          <Link href="/admin/dashboard" className="font-black lg:hidden">Command Center</Link>
          <div className="text-sm font-bold text-neutral-500">Shared admin panel at /admin</div>
          <Link href="/admin/login" className="rounded-lg border border-black/10 px-3 py-2 text-sm font-extrabold">Login</Link>
        </header>
        {children}
      </div>
    </div>
